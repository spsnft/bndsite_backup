...
const STATIC_FALLBACK_PATH = "/420/data/fallback-catalog.json";
...
async function fetchStaticFallback() {
  try {
    const res = await fetch(`${siteConfig.url}${STATIC_FALLBACK_PATH}`);
...
export async function getProducts() {
  ...
  // 2. Если не вышло — пробуем статический fallback
  const source = data || (await fetchStaticFallback());
  
  if (!source) {
    return { products: [], categories: {} };
  }

  const items: any[] = source.products || [];
  const formattedProducts = items.map(formatProduct);

  // 3. Группируем по категориям (data-driven)
